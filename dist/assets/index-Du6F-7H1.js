(function(){let e=document.createElement(`link`).relList;if(e&&e.supports&&e.supports(`modulepreload`))return;for(let e of document.querySelectorAll(`link[rel="modulepreload"]`))n(e);new MutationObserver(e=>{for(let t of e)if(t.type===`childList`)for(let e of t.addedNodes)e.tagName===`LINK`&&e.rel===`modulepreload`&&n(e)}).observe(document,{childList:!0,subtree:!0});function t(e){let t={};return e.integrity&&(t.integrity=e.integrity),e.referrerPolicy&&(t.referrerPolicy=e.referrerPolicy),e.crossOrigin===`use-credentials`?t.credentials=`include`:e.crossOrigin===`anonymous`?t.credentials=`omit`:t.credentials=`same-origin`,t}function n(e){if(e.ep)return;e.ep=!0;let n=t(e);fetch(e.href,n)}})();function e(e){return`https://splenedu52.github.io/g50/class-${e.id}/`}function t(e){return`/img/class-${e.id}.webp`}var n={Popular:`🔥`,New:`🆕`,Running:`🏃`,"3D":`🎲`,Shooting:`🎯`,Multiplayer:`👥`,Racing:`🏁`,Moto:`🏍️`,Stickman:`🥊`,Adventure:`🗺️`,Puzzle:`🧩`,Animal:`🐾`,Platform:`🕹️`,Simulation:`🖥️`,Management:`🏗️`,Survival:`🌿`,Strategy:`♟️`,Board:`♜`,Girls:`💖`,"2 Player":`🤝`,Car:`🏎️`,Sports:`⚽`,Skill:`🧠`},r=[{id:`all`,emoji:`🎮`,label:`🎮 All Games`,shortLabel:`All Games`},{id:`Popular`,emoji:`🔥`,label:`🔥 Popular`,shortLabel:`Popular`},{id:`New`,emoji:`🆕`,label:`🆕 New`,shortLabel:`New`},{id:`Skill`,emoji:`🧠`,label:`🧠 Skill`,shortLabel:`Skill`},{id:`Running`,emoji:`🏃`,label:`🏃 Running`,shortLabel:`Running`},{id:`Adventure`,emoji:`🗺️`,label:`🗺️ Adventure`,shortLabel:`Adventure`},{id:`Platform`,emoji:`🕹️`,label:`🕹️ Platform`,shortLabel:`Platform`},{id:`Car`,emoji:`🏎️`,label:`🏎️ Car`,shortLabel:`Car`},{id:`Racing`,emoji:`🏁`,label:`🏁 Racing`,shortLabel:`Racing`},{id:`Moto`,emoji:`🏍️`,label:`🏍️ Moto`,shortLabel:`Moto`},{id:`3D`,emoji:`🎲`,label:`🎲 3D`,shortLabel:`3D`},{id:`Shooting`,emoji:`🎯`,label:`🎯 Shooting`,shortLabel:`Shooting`},{id:`Multiplayer`,emoji:`👥`,label:`👥 Multiplayer`,shortLabel:`Multiplayer`},{id:`2 Player`,emoji:`🤝`,label:`🤝 2 Player`,shortLabel:`2 Player`},{id:`Sports`,emoji:`⚽`,label:`⚽ Sports`,shortLabel:`Sports`},{id:`Puzzle`,emoji:`🧩`,label:`🧩 Puzzle`,shortLabel:`Puzzle`},{id:`Animal`,emoji:`🐾`,label:`🐾 Animal`,shortLabel:`Animal`},{id:`Stickman`,emoji:`🥊`,label:`🥊 Stickman`,shortLabel:`Stickman`},{id:`Simulation`,emoji:`🖥️`,label:`🖥️ Simulation`,shortLabel:`Simulation`},{id:`Management`,emoji:`🏗️`,label:`🏗️ Management`,shortLabel:`Management`},{id:`Survival`,emoji:`🌿`,label:`🌿 Survival`,shortLabel:`Survival`},{id:`Strategy`,emoji:`♟️`,label:`♟️ Strategy`,shortLabel:`Strategy`},{id:`Board`,emoji:`♜`,label:`♜ Board`,shortLabel:`Board`},{id:`Girls`,emoji:`💖`,label:`💖 Girls`,shortLabel:`Girls`}],i={};function a(e,t){i[e]=t}function o(e){window.location.hash=e}function s(){let[e,...t]=(window.location.hash.slice(1)||`/`).split(`?`),n=Object.fromEntries(new URLSearchParams(t.join(`?`)));if(i[e]){i[e](n);return}for(let[t,r]of Object.entries(i))if(t.includes(`:`)){let i=RegExp(`^`+t.replace(/:([^/]+)/g,`([^/]+)`)+`$`),a=e.match(i);if(a){let e=[...t.matchAll(/:([^/]+)/g)].map(e=>e[1]),i=Object.fromEntries(e.map((e,t)=>[e,a[t+1]]));r({...n,...i});return}}i[`/`]&&i[`/`]({})}function c(){window.addEventListener(`hashchange`,s),s()}var l=`72aae8a75da17a34e48ed84feaa311bf`,u=`4b03159602cba0243869c415124b923e`;function d(e){return`<div class="ad-banner" style="display:flex;justify-content:center;margin:16px 0;background:rgba(255,255,255,0.02);border:1px dashed rgba(255,255,255,0.05);border-radius:12px;padding:8px 0;width:100%;overflow:hidden;"><iframe srcdoc="${`<!DOCTYPE html><html><head><style>body{margin:0;padding:0;display:flex;justify-content:center;align-items:center;background:transparent;overflow:hidden;}</style></head><body><script>window.atOptions={key:'${e}',format:'iframe',height:90,width:728,params:{}};<\/script><script src="https://biggerbreakerfind.com/${e}/invoke.js"><\/script></body></html>`.replace(/"/g,`&quot;`)}" width="728" height="90" frameborder="0" scrolling="no" style="display:block;margin:0;padding:0;border:none;"></iframe></div>`}var f=[],p=`all`,m=``,h=60,g=e=>document.getElementById(e);async function _(){f=await(await fetch(`/unblockgame/games.json`)).json()}function v(e){return e===`all`?f.length:f.filter(t=>(Array.isArray(t.cat)?t.cat:[t.cat]).includes(e)).length}function y(){let e=m.toLowerCase().trim();return f.filter(t=>{let n=Array.isArray(t.cat)?t.cat:[t.cat],r=p===`all`||n.includes(p),i=!e||t.title.toLowerCase().includes(e);return r&&i})}function b(){return`
  ...
  `,document.querySelectorAll(`.home-cat-chip`).forEach(e=>{e.addEventListener(`click`,t=>{t.preventDefault(),p=e.dataset.cat,h=60,T(),window.scrollTo({top:0,behavior:`smooth`})})}),D(),E()}function E(){let e=y(),i=g(`games-grid`),a=g(`game-count`),s=g(`load-more-wrap`),c=g(`section-title`);if(a&&(a.textContent=`(${e.length})`),c&&m)c.innerHTML=`🔍 Results for "<em>${m}</em>" <span class="count">(${e.length})</span>`;else if(c){let t=r.find(e=>e.id===p);c.innerHTML=`${p===`all`?`🔥 All Games`:t?.label||p} <span class="count">(${e.length})</span>`}if(e.length===0){i.innerHTML=`
      <div class="empty-state">
        <div class="empty-icon">🎮</div>
        <h3>No games found</h3>
        <p>Try a different search or category.</p>
        <button class="btn-primary" id="btn-clear-search">Clear Search</button>
      </div>`,s&&(s.style.display=`none`),g(`btn-clear-search`)?.addEventListener(`click`,()=>{m=``,g(`search-input`).value=``,g(`search-clear`).style.display=`none`,E()});return}i.innerHTML=e.slice(0,h).map((e,r)=>{let i=(Array.isArray(e.cat)?e.cat:[e.cat])[0]||``,a=n[i]||`🎮`;return`
      <article class="game-card" data-id="${e.id}" style="animation-delay:${Math.min(r,30)*20}ms" tabindex="0" role="button" aria-label="Play ${e.title}">
      ...
      </article>`}).join(``),s&&(s.style.display=e.length>h?`flex`:`none`),i.querySelectorAll(`.game-card`).forEach(e=>{let t=()=>o(`/play?id=${e.dataset.id}`);e.addEventListener(`click`,t),e.addEventListener(`keydown`,e=>{(e.key===`Enter`||e.key===` `)&&t()})})}function D(){g(`search-input`)?.addEventListener(`input`,e=>{m=e.target.value,g(`search-clear`).style.display=m?`flex`:`none`,h=60,E()}),g(`search-clear`)?.addEventListener(`click`,()=>{m=``,g(`search-input`).value=``,g(`search-clear`).style.display=`none`,E()}),document.querySelector(`.sidebar`)?.addEventListener(`click`,e=>{let t=e.target.closest(`.cat-item`);t&&(p=t.dataset.cat,h=60,document.querySelectorAll(`.cat-item`).forEach(e=>e.classList.remove(`active`)),t.classList.add(`active`),E(),g(`sidebar`)?.classList.remove(`open`),g(`sidebar-overlay`)?.classList.remove(`open`),window.scrollTo({top:0,behavior:`smooth`}))}),g(`btn-load-more`)?.addEventListener(`click`,()=>{h+=60,E()}),g(`btn-menu`)?.addEventListener(`click`,()=>{g(`sidebar`)?.classList.toggle(`open`),g(`sidebar-overlay`)?.classList.toggle(`open`)}),g(`sidebar-overlay`)?.addEventListener(`click`,()=>{g(`sidebar`)?.classList.remove(`open`),g(`sidebar-overlay`)?.classList.remove(`open`)})}function O({id:r}){let i=f.find(e=>e.id===r);if(!i){o(`/`);return}let a=e(i),s=Array.isArray(i.cat)?i.cat:[i.cat],c=s[0];document.title=`Play ${i.title} Unblocked | Unblocked Games G+`;let m=document.querySelector(`meta[name="description"]`);m||(m=document.createElement(`meta`),m.name=`description`,document.head.appendChild(m)),m.content=`Play ${i.title} unblocked free online on Unblocked Games G+. No download, no login. Free ${c} HTML5 game — works at school, work, or home on any device.`;let _=f.filter(e=>e.id!==i.id&&(Array.isArray(e.cat)?e.cat.includes(c):e.cat===c)).slice(0,12);g(`app`).innerHTML=`
    ${x()}
    <div class="layout">
      ${b()}
      <div class="main-content">
  ...
  `,g(`game-frame`)?.addEventListener(`load`,()=>{g(`game-loading`).style.display=`none`}),g(`btn-back`)?.addEventListener(`click`,()=>{window.history.back()}),g(`btn-fullscreen`)?.addEventListener(`click`,()=>{let e=g(`game-container`);document.fullscreenElement?document.exitFullscreen():e?.requestFullscreen()}),g(`btn-menu`)?.addEventListener(`click`,()=>{g(`sidebar`)?.classList.toggle(`open`),g(`sidebar-overlay`)?.classList.toggle(`open`)}),g(`sidebar-overlay`)?.addEventListener(`click`,()=>{g(`sidebar`)?.classList.remove(`open`),g(`sidebar-overlay`)?.classList.remove(`open`)}),document.querySelector(`.sidebar`)?.addEventListener(`click`,e=>{let t=e.target.closest(`.cat-item`);t&&(p=t.dataset.cat,h=60,o(`/`))}),document.getElementById(`related-grid`)?.querySelectorAll(`.game-card`).forEach(e=>{let t=()=>{h=60,o(`/play?id=${e.dataset.id}`)};e.addEventListener(`click`,t),e.addEventListener(`keydown`,e=>{e.key===`Enter`&&t()})})}a(`/`,()=>T()),a(`/play`,({id:e})=>O({id:e})),document.addEventListener(`DOMContentLoaded`,async()=>{g(`app`).innerHTML=`
